...
/*
 *  Removes all font tags generated by Google Translate from the given
 *	'.content_line' JQuery element
...
function removeFontTags(root){
	...
	$(root).find('.placeholder').each(function(){
		var placeholder_elem = this;
		placeholder_elem.normalize();

		placeholder_contents = $(this).contents().toArray();
		var place_before = true;
		$.each(placeholder_contents, function(){
			if (this.nodeType == Node.TEXT_NODE){
				if(place_before){
					$(this).insertBefore($(placeholder_elem));
				}
				else{
					$(this).insertAfter($(placeholder_elem));
				}
			}
			// Encountered the actual word anchor, flip the placement switch
			else{
				place_before = false;
			}
		});
	...
	// Finally wrap the now combined TextNodes in a <span>
